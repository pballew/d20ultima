...
func _ready():
	DebugLogger.info("=== F9 TEST SCRIPT STARTING ===")
	# Wait for game to fully initialize
	await get_tree().create_timer(5.0).timeout
	
	# Try to simulate pressing F9 key
	DebugLogger.info("Simulating F9 key press...")
	...
	DebugLogger.info("Simulating T key press...")
